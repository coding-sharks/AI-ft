...
2.6 **v3 部署对齐增强**(2026-07-09,治"麦克风音频不开口"的 OOD 问题;`--no-v3-aug` 可关):
   - **空档三态**(`GAP_STYLES`):衰减真噪声 45% / **数字绝对零 30%**(对齐浏览器
     noiseSuppression 的归零——实录中 2/3 帧是数字零,训练必须见过)/ 极安白噪底 25%;
   - **语音信道模拟**(`SPEECH_AUG_PROB=0.7`):对 TTS 语音随机施加 增益−8~+4dB /
     变速±10% / 轻混响 / 频谱倾斜+高通,缩小"干净合成音 vs 浏览器麦克风"的分布差;
   - **强烈建议配套"真实麦克风锚定集"**:用部署端 demo 实录 100~300 条真人口语问句
     (信道天然一致),配 LLM 回复混入训练并上采样 3-5×;另留 10-20 条作 held-out
     验收集——**重训后的验收指标 = 真实录音上语音结束点的 P(TEXT_BEGIN)**,
     训练分布上会开口不代表真实麦克风上会开口(2026-07-09 实测教训)。
2.7 **v4 连续噪声床**(2026-07-14,治"带噪真人声不开口";`--bed-prob 0` 可关):
   对照论文 StreamAudio-2M Algorithm 4——每条样本以 `BED_PROB=0.5` 概率改走"噪声床"
   样式:事件噪声轨**平铺整条音频(语音段上也叠)**,SNR~U(5,20)dB,另叠环境轨(再低
   5dB),接缝交叉淡化;床样式下空档=纯床,开口边界线索是**语音本身的消失**(床响度
   恒定),对部署端持续环境噪声免疫。其余 50% 保留 v3 三态空档(覆盖浏览器 NS 数字零
   形态)。动机:v1-v3 语音段全程干净,"人声+噪声"混合纹理是 OOD;守门日志新增
   `[床]` 与分样式 `[响度]` 两行。重训前先跑 `tools/make_snr_sweep.py` 确认实验
   (见 `TODO_v4_noisebed_H20.md`)。
