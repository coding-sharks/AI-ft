...
# ---- v4 连续噪声床(2026-07-14, 治"带噪真人声不开口") ----
# 对照论文 StreamAudio-2M Algorithm 4: 事件噪声轨【平铺整条音频, 语音段也叠】,
# SNR~U(5,20)dB(相对语音 RMS), 另叠一条再低 BED_AMBIENT_EXTRA_DB 的环境轨。
# 动机: v1-v3 的语音段始终是干净的, 部署端"人声+环境噪声"的混合纹理是 OOD;
# 且 v3 的开口线索依赖"空档极安静"(−18~30dB), 环境有持续噪声时线索永不出现——
# 噪声床样式的边界线索是【语音本身的消失】(床响度恒定), 对持续噪声免疫。
# 每条样本以 BED_PROB 概率走床样式(空档=纯床, 不再用三态), 其余走 v3 三态空档
# (对应浏览器 noiseSuppression 开启时的数字零形态, 两种部署形态都要覆盖)。
BED_PROB = 0.5
BED_SNR_DB_RANGE = (5.0, 20.0)
BED_AMBIENT_EXTRA_DB = 5.0
